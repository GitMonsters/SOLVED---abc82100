#!/usr/bin/env python3
    ...
    def find_indicators(
        ...
        H, W = dims(grid)
        all_cyan = set()
        for cl in clusters: all_cyan.update(cl)
        ...
        for ci, cluster in enumerate(clusters):
            ...
            for p1 in border:
                ...
                for p2 in adj4(p1[0], p1[1], H, W):
                    if p2 in used or p2 in all_cyan: continue
                    c2 = grid[p2[0]][p2[1]]
                    if c2 == 0 or c2 == 8 or c2 == c1: continue
                    ...
                    if total < best_dist:
                        best_dist = total
                        if d1 <= d2: best_pair = (p1, c1, p2, c2) # p1 closer
                        else: best_pair = (p2, c2, p1, c1) # p2 closer
    ...
    def solve(input_grid: Grid) -> Grid:
        ...
        clear: Set[Point] = set()
        for cl in clusters: clear.update(cl)
        ...
        for r, c in clear:
            if 0 <= r < H and 0 <= c < W: output[r][c] = 0
    ...
    def diff_grids(expected: Grid, actual: Grid) -> str:
        ...
        for r in range(H):
            ...
            act_row = "".join(EMOJI.get(actual[r][c], "?") for c in row)
            match = expected[r] == actual[r]
            marker = " ✓" if match else " ✗"
            if not match: mismatches += sum(1 for c in range(W) if expected[r][c] != actual[r][c])
